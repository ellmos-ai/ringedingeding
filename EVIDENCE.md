...
that was not run says so. Commands, dates, aggregate outcomes, and test counts
remain factual. Since the public privacy remediation on 2026-08-24, concrete
phone numbers, people, service identifiers, answers, timestamps, and transcript
fragments are explicitly synthetic reconstructions.
...
2. **The JSON shape of an `activity` entry** is unknown — only its rendered form
   (`00:00:00.000 | Callee said: …`, synthetically reconstructed) was ever seen.
...
The live retest of the R3/R12 package (`synthetic-poll-rt4c-a`,
`synthetic-poll-rt4c-b`)
...
## 37. RT-4c live retest — R18 through R21, final counted round — 2026-08-22/23

The identity-gate work (R18, R19), the retry data-loss fix (R20) and the
correction-call feature (R21) were all exercised together, live, in one
three-participant round represented here as `synthetic-poll-final` with neutral
people and options. Three
stages closed it out.

**Participant A — identity confirmed, counted after a clean retry.** The first attempt
was the one that triggered the R20 incident (see the R20 entries in
`AUFGABEN.txt` for the full account); after the R20(b) migration fix, the
retry placed a fresh call under a new attempt-scoped idempotency key with no
`409 idempotency_conflict`. The platform's voice agent asked the identity
question, and a synthetic option A, with a reason, was recorded and counted —
run `synthetic-call-final-a`.

**Participant B — skipped once, corrected once, counted in the end.** On the first
attempt the platform's voice agent skipped the CRITICAL identity question a
second time (identical goal text to Participant A's call, same round — agent-side
variance, not a wording problem: across four calls carrying this same goal
text, the platform followed the instruction twice and skipped it twice). The
synthetic answer was said clearly, twice, and the bot even echoed it back — but it was
still, correctly and conservatively, discarded: `reachable` stayed `false`,
and the report's Detail column carried the R18(b) explanation live instead of
sitting blank next to `COMPLETED`. Nothing was invented, and nothing was
lost. A day later, a `--retry` against exactly this state triggered the R21
correction call as designed: the agent asked, verbatim, whether it was
speaking with the right person, heard a yes, and — because the earlier
call's structured result carried no usable field to read back — asked the
substantive question fresh instead of quoting anything from the earlier
transcript, matching the documented fallback rather than guessing. The
answer came back cleanly structured this time and was counted — run
`synthetic-call-final-b`.

**The invitation round — first successful live invitation call.** A
separate, unrelated poll exercised the R5 invitation flow live for the first
time end to end: the identity question came first, a date and a time window
were spoken back in the person's own words rather than the offered wording,
and a clear acceptance was recorded and counted — run
`synthetic-call-invitation`.

Final state of `synthetic-poll-final`: every participant counted, one vote each
for synthetic option A and option B. One live round, evidence for R17 (serial calls
kept separate), R18(a) and R18(b) (the identity question live, and its
unconfirmed case reported honestly rather than left blank), R20 (a: no
downgrade of a counted answer was ever seen; b: the attempt-scoped key made
the retry succeed instead of colliding again; c: the already-counted answer
was never touched by a retry aimed at someone else), and R21 (the correction
call, including its designed fallback, ran exactly as built).
