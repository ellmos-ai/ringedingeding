...
def test_build_requests_refuses_participants_that_share_a_phone_number(store):
    ...
    assert not skipped
    assert not rejected
    assert {p.name for p, _reason in ambiguous} == {"Participant A", "Participant B"}
    ...
    assert refs_by_name["Participant B"] in reason_by_name["Participant A"]
    assert refs_by_name["Participant A"] in reason_by_name["Participant B"]
...
def test_build_requests_does_not_flag_a_number_shared_with_an_already_answered_person(store):
    ...
    poll = _poll_with_a_shared_number(store)
    people = store.participants(poll.id)
    already_answered = next(p for p in people if p.name == "Participant A")
    store.record_answer(
    ...
    assert {r.participant.name for r in requests} == {"Participant B", "Simon"}
...
def test_run_poll_never_dials_participants_sharing_a_phone_number(store):
    ...
    assert {p.name for p, _reason in report.ambiguous} == {"Participant A", "Participant B"}
