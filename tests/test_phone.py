...
@pytest.mark.parametrize(
    ...
    [
        ...
        "00447700900000",  # international prefix instead of '+'
        "+07700900123",  # country code may not start with zero
        "+1555",  # too short
...
def test_mask_text_scrubs_numbers_out_of_free_text():
    text = "Call me back on +1 555 555 0100 or +442079460000, thanks"
    scrubbed = mask_text(text)
    assert "5555550100" not in scrubbed
    assert "2079460000" not in scrubbed
