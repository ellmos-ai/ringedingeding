...
def test_the_observed_log_parses_into_speakers_and_text():
    ...
    assert lines[0].timestamp == "17:37:45.146"


def test_the_prefix_is_stripped_but_the_original_line_is_kept():
    line = parse_activity(["17:37:51.577 | Callee said: hallo"])[0]
...
def test_dictionary_entries_work_too():
    ...
    lines = parse_activity(
        [
            {"timestamp": "17:37:45.146", "message": "Call is ringing."},
            {"time": "17:37:50.769", "text": "Bot is speaking: Hello."},
            {"at": "17:37:51.577", "speaker": "callee", "text": "hallo"},
        ]
    )
    assert [line.speaker for line in lines] == [Speaker.SYSTEM, Speaker.BOT, Speaker.CALLEE]
    assert lines[1].text == "Hello."
    assert lines[2].timestamp == "17:37:51.577"
...
def test_a_streamed_correction_replaces_its_rough_version():
    ...
    assert spoken == ["Hallo.", "2. Ja, unzufrieden."]
...
def test_a_correction_that_only_grows_the_sentence_is_folded_too():
    lines = dedupe(
        parse_activity(
            [
                "17:38:15.100 | Callee said: 2. Ja",
                "17:38:15.892 | Callee said: 2. Ja, unzufrieden.",
            ]
        )
    )
    assert [line.text for line in lines] == ["2. Ja, unzufrieden."]
...
def test_the_display_line_masks_phone_numbers():
    line = parse_activity(["17:37:51.577 | Callee said: Ruf mich auf +49 151 12345678 an"])[0]
