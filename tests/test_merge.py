...
def test_two_participants_sharing_a_run_id_are_counted_as_one_vote():
    ...
    assert merged.coverage.shared_call[0].shares_call_with == "Participant A"
...
def test_shared_call_caveat_names_both_participants():
    ...
    assert caveat is not None
    assert "Participant B" in caveat
    assert "shares a call with Participant A" in caveat
